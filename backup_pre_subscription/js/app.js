document.addEventListener('DOMContentLoaded', () => {
    ...
    voice = new VoiceRecognizer(
        ...
        (text) => {
            ...
            if (chatOverlay.classList.contains('active')) {
                ...
                // If spoken from Home screen, parse to see if it is complete or has doubts
                const result = parser.parseText(text);
                if (result.hasDoubt) {
                    // Incomplete: open chat to guide user with chips/options
                    openChatOverlay();
                    ...
                    // Complete: process directly in the background (no chat overlay opened)
                    storage.addChatMessage('user', text);
                    renderChatMessages();
                    processChatAssistantReply(text);
                }
    ...
    // Mic click on Home starts recording directly on the home page (turns red and pulses)
                ...
                if (!isNaN(parsedVal) && parsedVal > 0) {
                    ...
                    if (doubtTx.isFixedAccountTemplate) {
                        ...
                        if (doubtTx.cofrinhoAction === 'guardar') {
                            ...
                            addAppMessage(`Entendido! Guardei <strong>R$ ${parsedVal.toFixed(2).replace('.', ',')}</strong> no seu cofrinho. 🐷💰`, false);
                        } else {
                            storage.withdrawFromCofrinho(parsedVal);
                            addAppMessage(`Entendido! Resgatei <strong>R$ ${parsedVal.toFixed(2).replace('.', ',')}</strong> do seu cofrinho. 🐷💵`, false);
                        }
                        storage.setActiveConversationState({ status: 'idle' });
                    } else {
                        let reconText = "";
            ...
            if (result.hasDoubt) {
                ...
                if (result.isFixedAccountTemplate) {
                    ...
                    if (result.cofrinhoAction === 'guardar') {
                        ...
                        addAppMessage(`Entendido! Guardei <strong>R$ ${result.value.toFixed(2).replace('.', ',')}</strong> no seu cofrinho. 🐷💰`, false);
                    } else if (result.cofrinhoAction === 'resgatar') {
                        storage.withdrawFromCofrinho(result.value);
                        addAppMessage(`Entendido! Resgatei <strong>R$ ${result.value.toFixed(2).replace('.', ',')}</strong> do seu cofrinho. 🐷💵`, false);
                    }
                } else {
